...
export class RequestValidator {
  ...
  static _invalidUploadMetadataHeader(value: string) {
    const keypairs = value.split(',').map(keypair => keypair.trim())
  ...
  static capitalizeHeader(headerName: string) {
    return headerName
  ...
  static isInvalidHeader(this: any, headerName: string, headerValue: string) {
    if (HEADERS_LOWERCASE.indexOf(headerName) === -1) {
      return false
    }
